...
temp_path = r'model_parameter/diff_gprnet_temp.pth'

# Define the path to save the best model
best_path = r'model_parameter/diff_gprnet_best.pth'
# Define the log file path
log_file_path = r'training_log_diff_gprnet.txt'
# ------------------------------------------
dataset = MyDataset_d(
...
def save_training_log(epoch, train_loss, eval_loss, lr, file_path):
    """Save training information to txt file"""
    with open(file_path, 'a') as f:
        if epoch == 1:
            # If it's the first epoch, write the header first
            f.write("Epoch,Train_Loss,Eval_Loss,Learning_Rate,Timestamp\n")
        
        timestamp = time.strftime("%Y-%m-%d %H:%M:%S")
        f.write(f"{epoch},{train_loss:.6f},{eval_loss:.6f},{lr:.2e},{timestamp}\n")
...
if __name__ == "__main__":
    ...
    for epoch in range(start_epoch + 1, epochs + start_epoch + 1):
        ...
        save_training_log(epoch, avg_train_loss, avg_eval_loss, current_lr, log_file_path)
        ...
        # Save best model - based on Eval Loss only
        if avg_eval_loss < best_eval:
            ...
            print(f"  Best metrics - Loss: {best_eval:.6f}")
...
plt.tight_layout()
plt.savefig('training_loss_model_parameter.png')
plt.show()
